...
import { LabelImageDataUtil } from '../LabelImageDataUtil';
import { AcceptedFileType } from '../../data/enums/AcceptedFileType';


const getDummyImageData = (id: string): LabelImageData => {
    return {
        id,
...
describe('ImageDataUtil cleanAnnotation method', () => {
    it('should return new ImageData object without annotations', () => {
        // given
        const item: LabelImageData = getDummyImageData(uuidv4());
        item.labelRects = [
        ...
        // when
        const result = LabelImageDataUtil.cleanAnnotations(item);
...
describe('ImageDataUtil arrange method', () => {
    it('should return new array with correctly arranged ImageData objects', () => {
        ...
        // when
        const result = LabelImageDataUtil.arrange(items, expectedIdArrangement);
        const resultIdArrangement = result.map((item: LabelImageData) => item.id)
